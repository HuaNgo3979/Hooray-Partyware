...
- **Run the SQL:** load [`hp_sql_scripts.sql`](HP-Database-Design-Assessment3/sql/hp_sql_scripts.sql) into a MySQL client against a database that implements the `Customer`, `Invoice`, `InvoiceDetail`, and `Product` schema described above, or read through it for docx file [`SQL-Script`](HP-Database-Design-Assessment3/docs/SQL-Scripts.sql).
